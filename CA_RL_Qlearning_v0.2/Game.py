...
class Game():
    ...
    def drawMap(self):
        # around wall
        plt.plot([0, ROOM_M], [0, 0], 'k-')  # down
        plt.plot([ROOM_M / 2 - 2, 0], [ROOM_M, ROOM_M], 'k-')  # up
        plt.plot([ROOM_M, ROOM_M / 2 + 2], [ROOM_M, ROOM_M], 'k-')  # up
        plt.plot([0, 0], [0, ROOM_M], 'k-')  # left
        plt.plot([ROOM_M, ROOM_M], [0, ROOM_M], 'k-')  # right
        # exit
        plt.plot([ROOM_M / 2 - 2, ROOM_M / 2 + 2], [ROOM_M, ROOM_M], 'b--')
    ...
    def isCanMove(self, p, direction):
        ...
        if p_x_ < ROOM_M / 2 + 2 and p_x_ > ROOM_M / 2 - 2 and p_y_ == ROOM_M:
            print("the pedestrian in exit")
            isGetExit = True
        if p_x_ < ROOM_M / 2 + 2 and p_x_ > ROOM_M / 2 - 2 and p_y_ == ROOM_M-1:
            self.isNearExit=True
        if p_x_ <= 0 or p_x_ >= ROOM_M or p_y_ <= 0 or p_y_ >= ROOM_M:
    ...
    def initRandomPed(self):
        allBlock = []
        allPeople = []
        for i in range(1, ROOM_M):
            for j in range(1, ROOM_M):
                b = Block()
                b.x = i
                b.y = j

        random.shuffle(allBlock)
        allPeople = allBlock[:PEOPLE_NUMBER]
        return allPeople
